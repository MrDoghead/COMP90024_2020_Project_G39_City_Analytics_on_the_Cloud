...
def aurin_map(aurinIncome):
    for data in aurinIncome:
        loc=data["loc"]
        inc=data["tolIncome"]
        if inc<q:
            folium.Marker(location=loc, popup='Add popup text here.',icon=folium.Icon(color="red"),).add_to(m3)
        if inc>=q:
            folium.Marker(location=loc, popup='Add popup text here.',icon=folium.Icon(color="green"),).add_to(m3)
    return m3

aurin_map(aurinIncome).save('/home/ubuntu/project/flask/static/AurinMap_Income.html')